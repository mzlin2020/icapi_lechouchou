...
class AnchorsService {
  ...
  async getAnchorsInfo(offset, size, anchorName, fansNum) {
    ...
    if (!offset && !size) {
      ...
      const result = await connection.execute(statement, [anchorName, fansNum[0],fansNum[1],anchorName, fansNum[0],fansNum[1],offset, size]);
      return result;
      ...
      `;
      // 有传值，则根据传值获取数据
      const result = await connection.execute(statement, [fansNum[0],fansNum[1],fansNum[0],fansNum[1],offset,size,]);
      return result;
    } else {
      // 有传值，则根据传值获取数据
